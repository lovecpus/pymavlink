...
MAVLinkSigning = function MAVLinkSigning(object){ 
        this.secret_key = Buffer.from([]) ; //Buffer.from([ 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42, 42 ]) // secret key must be a Buffer obj of 32 length 
        this.timestamp = 1 
        this.link_id = 0 
...
${MAVPROCESSOR}.prototype.check_signature = function(msgbuf, srcSystem, srcComponent) { 
        ...
        //link_id = msgbuf[-13] 
        var link_id= Buffer.from(msgbuf.slice(-13,-12)); // just a single byte really, but returned as a buffer 
        ...
        // I means unsigned 4bytes, H means unsigned 2 bytes
        var t = jspack.Unpack('<IH',Buffer.from(timestamp_buf))  
        const [tlow, thigh]  = t; 
        ...
        sigpart = Buffer.from(sigpart); 
        // not sig part 0- end-minus-6 
        var notsigpart = msgbuf.slice(0,-6);  
        notsigpart = Buffer.from(notsigpart); 
...
// if(typeof module === "object" && module.exports) { module.exports = {${MAVHEAD}, ${MAVPROCESSOR}}; }
    ...
    for m in msgs:
        ...
        for f in m.ordered_fieldnames:
            ...
            if _isarray and (fieldtype == 'char'):
                tdata = 'Buffer.from('+m.test_data[idx]+').toString("binary")'; # binary encoding here is important for bits >= 128
            ...
            # array of other things
                ...
                tdata = 'Buffer.from('+m.test_data[idx]+') // generic buffer error?';  
        ...
        outf.write(" //var t = Buffer.from([])\n; //%s\n"% (  m.name.lower()));
        outf.write(" var t = Buffer.from(test_%s.pack(mav));\n"% (  m.name.lower()));
